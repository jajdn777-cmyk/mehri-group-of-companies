  ...
  try {
    ...
    if (action === 'SYNC_USER') {
      ...
      const [p, w, g, r, m, b, c, am, ac] = await Promise.all([
         ...
         supabase.from('calorie-ai_memories').select('memory_text').eq('user_id', userId),
         supabase.from('calorie-ai_chats').select('*').eq('user_id', userId)
      ]);
      ...
      return {
        ...
        data: {
          ...
          'calorie-ai': { memories: am.data ? am.data.map((x:any) => x.memory_text) : [], chats: ac.data || [] }
        }
      };
    }
    ...
    if (action === 'SAVE_SESSION') {
       if(!userId) return { status: 'error', message: 'Not logged in' };
       const { data } = await supabase.from('calorie-ai_chats').select('id').eq('id', payload.id).maybeSingle();
       let error;
       if (data) {
          const res = await supabase.from('calorie-ai_chats').update({
             title: payload.title,
          ...
          const res = await supabase.from('calorie-ai_chats').insert({
             id: payload.id,
             user_id: userId,
             title: payload.title,
             messages: payload.messages,
    ...
    if (action === 'DELETE_SESSION') {
       await supabase.from('calorie-ai_chats').delete().eq('id', payload.id);
       return { status: 'success' };
    }

    if (action === 'SAVE_MEMORIES') {
       if(!userId) return { status: 'error', message: 'Not logged in' };
       await supabase.from('calorie-ai_memories').delete().eq('user_id', userId);
       const mems = payload.memories.map((m: string) => ({ user_id: userId, memory_text: m }));
       if (mems.length > 0) {
           const { error } = await supabase.from('calorie-ai_memories').insert(mems);
           if (error) return { status: 'error', message: error.message };
       }
       return { status: 'success' };
    }
