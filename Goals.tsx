  ...
  return (
    <div className="fixed inset-0 z-[6000] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md overflow-y-auto" role="dialog" aria-modal="true">
      <div className="bg-white p-6 md:p-10 rounded-[30px] md:rounded-[50px] shadow-2xl w-full max-w-4xl relative overflow-visible animate-scale-in my-8">
        <button onClick={onClose} className="absolute top-6 right-6 md:top-8 md:right-8 text-slate-300 hover:text-emerald-500 transition-colors focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none" aria-label="Close modal"><X size={28} aria-hidden="true"/></button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          <div className="space-y-3 relative md:z-50">
            ...
            <div className="relative">
              <button type="button" id={activityId} className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-base flex items-center justify-between cursor-pointer hover:bg-slate-100 transition-colors focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none" onClick={() => setIsDropdownOpen(!isDropdownOpen)} aria-haspopup="listbox" aria-expanded={isDropdownOpen}>
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-100 rounded-[25px] shadow-2xl overflow-hidden h-64 flex flex-col z-[100]" role="listbox">
                  <div className="p-3 border-b border-slate-50 bg-white">
                    <div className="flex items-center bg-slate-50 rounded-xl px-3 py-2 focus-within:ring-2 ring-emerald-500">
                      ...
                      <input id={searchId} className="bg-transparent w-full font-bold text-xs outline-none" placeholder="Search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} autoFocus aria-label="Search activity types" />
                    </div>
                  </div>
          ...
          <div className="space-y-3 z-40">
            <label htmlFor={goalTypeId} className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest cursor-pointer">Goal Type</label>
            <div className="flex gap-2">
               {['count', 'distance', 'duration'].map((t, idx) => (
                 <button key={t} id={idx === 0 ? goalTypeId : undefined} onClick={() => setGoalType(t as any)} className={`flex-1 py-5 rounded-[25px] font-black uppercase text-[10px] tracking-widest transition-all focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none ${goalType === t ? 'bg-slate-900 text-white shadow-lg' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}>
                   {t === 'count' ? '# Workouts' : t}
                 </button>
               ))}
            </div>
          ...
          <div className="space-y-3 z-30">
            ...
            {goalType === 'duration' ? (
              <div className="flex gap-2">
                 <input id={targetId} className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus-visible:ring-2 ring-emerald-500 outline-none" placeholder="HH" value={durationDetails.h} onChange={e => setDurationDetails({...durationDetails, h: e.target.value})} aria-label="Hours" />
                 <span className="self-center font-black text-slate-300" aria-hidden="true">:</span>
                 <input className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus-visible:ring-2 ring-emerald-500 outline-none" placeholder="MM" value={durationDetails.m} onChange={e => setDurationDetails({...durationDetails, m: e.target.value})} aria-label="Minutes" />
                 <span className="self-center font-black text-slate-300" aria-hidden="true">:</span>
                 <input className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus-visible:ring-2 ring-emerald-500 outline-none" placeholder="SS" value={durationDetails.s} onChange={e => setDurationDetails({...durationDetails, s: e.target.value})} aria-label="Seconds" />
              </div>
            ) : (
              <input id={targetId} type="number" className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-lg focus-visible:ring-2 ring-emerald-500 outline-none" placeholder={goalType === 'distance' ? "e.g. 50" : "e.g. 10"} value={target} onChange={e => setTarget(e.target.value)} />
            )}
          </div>
          <div className="space-y-3 z-30">
            ...
            <input id={startDateId} type="date" className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-slate-900 focus-visible:ring-2 ring-emerald-500 outline-none" value={startDate} onChange={e => setStartDate(e.target.value)} />
          </div>
        </div>
  ...
  return (
    <div className="space-y-12 md:space-y-16 animate-fade-in pb-32 px-4 md:px-0">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end border-b border-slate-100 pb-8 gap-8">
        ...
        <button onClick={() => setShowCreate(true)} className="w-full md:w-auto bg-emerald-400 text-slate-900 px-10 py-5 rounded-[30px] font-black uppercase text-[10px] tracking-[0.3em] hover:scale-105 transition-all shadow-xl flex items-center justify-center gap-3 focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none">
      ...
      <div className="space-y-8">
        <div className="flex justify-between items-center">
           <div className="flex gap-4">
              <button onClick={() => setFilter('Active')} className={`text-[10px] font-black uppercase tracking-widest transition-colors focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none rounded-sm ${filter === 'Active' ? 'text-emerald-500' : 'text-slate-300'}`}>Active</button>
              <button onClick={() => setFilter('Completed')} className={`text-[10px] font-black uppercase tracking-widest transition-colors focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none rounded-sm ${filter === 'Completed' ? 'text-emerald-500' : 'text-slate-300'}`}>Completed</button>
           </div>
        </div>
        ...
        {visibleGoals.length > 0 ? (
               ...
               <div key={g.id} className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-lg flex flex-col justify-between h-56 relative overflow-hidden group">
                  ...
                  <div>
                    <div className="flex justify-between items-start mb-2">
                       ...
                       {filter === 'Active' && stats.isComplete && (
                          <button onClick={() => markCompleted(g)} className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest hover:bg-emerald-200 transition-colors flex items-center gap-1 focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none">
                  ...
                  <div className="space-y-2 mt-4 relative z-10">
                    ...
                    <div className="flex justify-between items-end">
                        ...
                        <button onClick={(e) => {e.stopPropagation(); deleteGoal(g.id);}} className="text-slate-300 hover:text-red-500 p-2 rounded-full hover:bg-red-50 transition-all z-20 focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none" aria-label="Delete goal"><Trash2 size={16} aria-hidden="true"/></button>
                    </div>
                  </div>
               </div>
             )})}
          ...
          <div className="w-full py-20 border-2 border-dashed border-slate-200 rounded-[40px] flex flex-col items-center justify-center text-center space-y-4">
            ...
            {filter === 'Active' && (
                <button onClick={() => setShowCreate(true)} className="mt-4 text-[10px] font-black uppercase tracking-widest text-emerald-500 hover:text-emerald-600 underline focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none rounded-sm">Create First Goal</button>
            )}
          </div>
        )}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <div key={i} className="bg-white p-8 md:p-10 rounded-[40px] md:rounded-[50px] border border-slate-100 shadow-sm flex flex-col gap-6 hover:shadow-xl transition-all hover:-translate-y-1">
               ...
               <button onClick={() => addPreset(preset.count)} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] tracking-[0.3em] hover:bg-emerald-400 transition-colors focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none">Set Goal</button>
            </div>
          ))}
        </div>
      </div>
