...
const CreateGoalModal = ({ onClose, onSubmit, userPreferences }: any) => {
  ...
  const flattenedActivities = useMemo(() => {
    const list: string[] = ['Any Activity'];
    Object.values(filteredCategories).forEach(acts => {
      list.push(...acts);
    });
    return list;
  }, [filteredCategories]);
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'ArrowDown') {
      setFocusedIndex(prev => (prev < flattenedActivities.length - 1 ? prev + 1 : prev));
      e.preventDefault();
    } else if (e.key === 'ArrowUp') {
      setFocusedIndex(prev => (prev > 0 ? prev - 1 : prev));
      e.preventDefault();
    } else if (e.key === 'Enter' && focusedIndex >= 0) {
      setActivity(flattenedActivities[focusedIndex]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-100 rounded-[25px] shadow-2xl overflow-hidden h-64 flex flex-col z-[100]">
                  <div className="p-3 border-b border-slate-50 bg-white">
                    <div className="flex items-center bg-slate-50 rounded-xl px-3 py-2">
                      ...
                      <input
                        className="bg-transparent w-full font-bold text-xs outline-none"
                        placeholder="Search..."
                  ...
                  <div role="listbox" className="overflow-y-auto custom-scrollbar p-2 flex-1">
                     <button
                       role="option"
                       aria-selected={activity === 'Any Activity'}
                       className={`w-full text-left px-4 py-2 rounded-lg text-xs font-bold transition-colors ${focusedIndex === 0 ? 'bg-emerald-50 text-emerald-600' : 'hover:bg-slate-50 text-emerald-600'} mb-2`}
                       onClick={() => { setActivity('Any Activity'); setIsDropdownOpen(false); }}
                     >
                       ★ Any Activity
                     </button>
                    {Object.keys(filteredCategories).map((category) => (
                      <div key={category} className="mb-3">
                        ...
                        {filteredCategories[category].map((act) => {
                          const index = flattenedActivities.indexOf(act);
                          return (
                            <button
                              key={act}
                              role="option"
                              aria-selected={activity === act}
                              className={`w-full text-left px-4 py-2 rounded-lg text-xs font-bold transition-colors ${focusedIndex === index ? 'bg-slate-100 text-slate-900' : 'hover:bg-slate-50 text-slate-600'}`}
                              onClick={() => { setActivity(act); setIsDropdownOpen(false); }}
                            >
                              {act}
                            </button>
                          );
          ...
          <div className="space-y-3 z-40">
            ...
            <div className="flex gap-2">
               {['count', 'distance', 'duration'].map((t, i) => (
                 <button
                   key={t}
                   id={i === 0 ? goalTypeId : undefined}
          ...
          <div className="space-y-3 z-30">
            ...
            {goalType === 'duration' ? (
              <div className="flex gap-2">
                 <input id={targetId} className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none" placeholder="HH" aria-label="Hours" value={durationDetails.h} onChange={e => setDurationDetails({...durationDetails, h: e.target.value})} />
                 <span className="self-center font-black text-slate-300" aria-hidden="true">:</span>
                 <input className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none" placeholder="MM" aria-label="Minutes" value={durationDetails.m} onChange={e => setDurationDetails({...durationDetails, m: e.target.value})} />
                 <span className="self-center font-black text-slate-300" aria-hidden="true">:</span>
                 <input className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none" placeholder="SS" aria-label="Seconds" value={durationDetails.s} onChange={e => setDurationDetails({...durationDetails, s: e.target.value})} />
              </div>
            ) : (
              <input id={targetId} type="number" className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-lg focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none" placeholder={goalType === 'distance' ? "e.g. 50" : "e.g. 10"} value={target} onChange={e => setTarget(e.target.value)} />
            )}
          </div>
          <div className="space-y-3 z-30">
            ...
            <input id={startDateId} type="date" className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-slate-900 focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none" value={startDate} onChange={e => setStartDate(e.target.value)} />
          </div>
        </div>
      ...
      <div className="space-y-8">
        <div className="flex justify-between items-center">
           <div className="flex gap-4">
              <button onClick={() => setFilter('Active')} className={`text-[10px] font-black uppercase tracking-widest transition-colors focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none rounded-lg px-2 py-1 ${filter === 'Active' ? 'text-emerald-500' : 'text-slate-300'}`}>Active</button>
              <button onClick={() => setFilter('Completed')} className={`text-[10px] font-black uppercase tracking-widest transition-colors focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none rounded-lg px-2 py-1 ${filter === 'Completed' ? 'text-emerald-500' : 'text-slate-300'}`}>Completed</button>
           </div>
        </div>
        ...
        {visibleGoals.length > 0 ? (
               ...
               <div key={g.id} className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-lg flex flex-col justify-between h-56 relative overflow-hidden group">
                  ...
                  <div className="space-y-2 mt-4 relative z-10">
                    ...
                    <div className="flex justify-between items-end">
                        ...
                        <button onClick={(e) => {e.stopPropagation(); deleteGoal(g.id);}} aria-label="Delete goal" className="text-slate-300 hover:text-red-500 p-2 rounded-full hover:bg-red-50 transition-all z-20 focus-visible:ring-2 ring-red-500 ring-offset-2 outline-none"><Trash2 size={16} aria-hidden="true" /></button>
                    </div>
                  </div>
               </div>
             )})}
          ...
          <div className="w-full py-20 border-2 border-dashed border-slate-200 rounded-[40px] flex flex-col items-center justify-center text-center space-y-4">
            ...
            {filter === 'Active' && (
                <button onClick={() => setShowCreate(true)} className="mt-4 text-[10px] font-black uppercase tracking-widest text-emerald-500 hover:text-emerald-600 underline focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none rounded px-2">Create First Goal</button>
            )}
          </div>
        )}
      </div>
