      ...
      <div className="bg-white p-6 md:p-10 rounded-[30px] md:rounded-[50px] shadow-2xl w-full max-w-4xl relative overflow-visible animate-scale-in my-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-100 rounded-[25px] shadow-2xl overflow-hidden h-64 flex flex-col z-[100]">
                  <div className="p-3 border-b border-slate-50 bg-white">
                    <div className="flex items-center bg-slate-50 rounded-xl px-3 py-2">
                      ...
                      <input
                        className="bg-transparent w-full font-bold text-xs outline-none focus-visible:ring-2 ring-emerald-500 rounded-sm"
                        placeholder="Search..."
                        value={searchTerm}
          ...
          <div className="space-y-3 z-40">
            ...
            <div id={goalTypeId} className="flex gap-2">
               {['count', 'distance', 'duration'].map(t => (
                 <button key={t} type="button" onClick={() => setGoalType(t as any)} className={`flex-1 py-5 rounded-[25px] font-black uppercase text-[10px] tracking-widest transition-all focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none ${goalType === t ? 'bg-slate-900 text-white shadow-lg' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}>
                   {t === 'count' ? '# Workouts' : t}
                 </button>
               ))}
            </div>
          ...
          <div className="space-y-3 z-30">
            ...
            {goalType === 'duration' ? (
              <div id={targetId} className="flex gap-2">
                 <input className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus-visible:ring-2 ring-emerald-500 outline-none" placeholder="HH" value={durationDetails.h} onChange={e => setDurationDetails({...durationDetails, h: e.target.value})} aria-label="Hours" />
