  ...
  return (
    <div className="fixed inset-0 z-[6000] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md overflow-y-auto" role="dialog" aria-modal="true">
      <div className="bg-white p-6 md:p-10 rounded-[30px] md:rounded-[50px] shadow-2xl w-full max-w-4xl relative overflow-visible animate-scale-in my-8">
        <button onClick={onClose} aria-label="Close modal" className="absolute top-6 right-6 md:top-8 md:right-8 text-slate-300 hover:text-emerald-500 transition-colors focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none rounded-lg"><X size={28} aria-hidden="true"/></button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          <div className="space-y-3 relative md:z-50">
            ...
            <div className="relative">
              <button id={activityId} type="button" aria-haspopup="listbox" aria-expanded={isDropdownOpen} className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-base flex items-center justify-between cursor-pointer hover:bg-slate-100 transition-colors focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none" onClick={() => setIsDropdownOpen(!isDropdownOpen)}>
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-100 rounded-[25px] shadow-2xl overflow-hidden h-64 flex flex-col z-[100]" role="listbox">
                  <div className="p-3 border-b border-slate-50 bg-white">
                    <div className="flex items-center bg-slate-50 rounded-xl px-3 py-2">
                      ...
                      <input aria-label="Search activity types" className="bg-transparent w-full font-bold text-xs outline-none" placeholder="Search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} autoFocus />
                    </div>
                  </div>
          ...
          <div className="space-y-3 z-40">
            <label htmlFor={typeId} className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest cursor-pointer">Goal Type</label>
            <div className="flex gap-2">
               {['count', 'distance', 'duration'].map((t, idx) => (
                 <button id={idx === 0 ? typeId : undefined} key={t} onClick={() => setGoalType(t as any)} className={`flex-1 py-5 rounded-[25px] font-black uppercase text-[10px] tracking-widest transition-all focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none ${goalType === t ? 'bg-slate-900 text-white shadow-lg' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}>
                   {t === 'count' ? '# Workouts' : t}
                 </button>
               ))}
            </div>
          ...
          <div className="space-y-3 z-30">
            ...
            {goalType === 'duration' ? (
              <div className="flex gap-2">
                 <input id={targetId} aria-label="Hours" className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus:ring-2 ring-emerald-500/20 outline-none" placeholder="HH" value={durationDetails.h} onChange={e => setDurationDetails({...durationDetails, h: e.target.value})} />
                 <span className="self-center font-black text-slate-300" aria-hidden="true">:</span>
                 <input aria-label="Minutes" className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus:ring-2 ring-emerald-500/20 outline-none" placeholder="MM" value={durationDetails.m} onChange={e => setDurationDetails({...durationDetails, m: e.target.value})} />
                 <span className="self-center font-black text-slate-300" aria-hidden="true">:</span>
                 <input aria-label="Seconds" className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus:ring-2 ring-emerald-500/20 outline-none" placeholder="SS" value={durationDetails.s} onChange={e => setDurationDetails({...durationDetails, s: e.target.value})} />
              </div>
            ) : (
              <input id={targetId} type="number" className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-lg focus:ring-2 ring-emerald-500/20 outline-none" placeholder={goalType === 'distance' ? "e.g. 50" : "e.g. 10"} value={target} onChange={e => setTarget(e.target.value)} />
            )}
          </div>
          <div className="space-y-3 z-30">
            <label htmlFor={dateId} className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest cursor-pointer">Start Date</label>
            <input id={dateId} type="date" className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-slate-900 focus:ring-2 ring-emerald-500/20 outline-none" value={startDate} onChange={e => setStartDate(e.target.value)} />
          </div>
        </div>
               ...
               <div key={g.id} className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-lg flex flex-col justify-between h-56 relative overflow-hidden group">
                  ...
                  <div className="space-y-2 mt-4 relative z-10">
                    ...
                    <div className="flex justify-between items-end">
                        ...
                        <button onClick={(e) => {e.stopPropagation(); deleteGoal(g.id);}} aria-label="Delete goal" className="text-slate-300 hover:text-red-500 p-2 rounded-full hover:bg-red-50 transition-all z-20 focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none"><Trash2 size={16} aria-hidden="true" /></button>
                    </div>
                  </div>
               </div>
             )})}
