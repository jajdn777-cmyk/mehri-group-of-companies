...
const CreateGoalModal = ({ onClose, onSubmit, userPreferences }: any) => {
  ...
  return (
    <div className="fixed inset-0 z-[6000] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md overflow-y-auto" role="dialog" aria-modal="true">
      <div className="bg-white p-6 md:p-10 rounded-[30px] md:rounded-[50px] shadow-2xl w-full max-w-4xl relative overflow-visible animate-scale-in my-8">
        <button onClick={onClose} aria-label="Close modal" className="absolute top-6 right-6 md:top-8 md:right-8 text-slate-300 hover:text-emerald-500 transition-colors focus-visible:ring-2 ring-emerald-500 outline-none rounded-full"><X size={28} aria-hidden="true"/></button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          <div className="space-y-3 relative md:z-50">
            <label htmlFor={activityId} className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest">Activity</label>
            <div className="relative">
              <button
                ...
                className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-base flex items-center justify-between cursor-pointer hover:bg-slate-100 transition-colors focus-visible:ring-2 ring-emerald-500 outline-none"
              ...
              {isDropdownOpen && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-100 rounded-[25px] shadow-2xl overflow-hidden h-64 flex flex-col z-[100]" role="listbox">
                  <div className="p-3 border-b border-slate-50 bg-white">
                    <div className="flex items-center bg-slate-50 rounded-xl px-3 py-2 focus-within:ring-2 ring-emerald-500">
                      <Search size={14} className="text-slate-400 mr-2" aria-hidden="true"/>
                      <input className="bg-transparent w-full font-bold text-xs outline-none" aria-label="Search activities" placeholder="Search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} autoFocus />
                    </div>
                  </div>
                  <div className="overflow-y-auto custom-scrollbar p-2 flex-1">
                     <button role="option" aria-selected={activity === 'Any Activity'} className="w-full text-left px-4 py-2 rounded-lg text-xs font-bold hover:bg-slate-50 text-emerald-600 mb-2 focus-visible:ring-2 ring-emerald-500 outline-none" onClick={() => { setActivity('Any Activity'); setIsDropdownOpen(false); }}>★ Any Activity</button>
                    {Object.keys(filteredCategories).map(category => (
                      <div key={category} className="mb-3">
                        ...
                        {filteredCategories[category].map(act => (
                          <button key={act} role="option" aria-selected={activity === act} className="w-full text-left px-4 py-2 rounded-lg text-xs font-bold hover:bg-slate-50 text-slate-600 focus-visible:ring-2 ring-emerald-500 outline-none" onClick={() => { setActivity(act); setIsDropdownOpen(false); }}>{act}</button>
                        ))}
                      </div>
                    ))}
                  </div>
          ...
          <div className="space-y-3 z-40">
            <label htmlFor={goalTypeId} className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest">Goal Type</label>
            <div className="flex gap-2" id={goalTypeId}>
               {['count', 'distance', 'duration'].map(t => (
                 <button key={t} type="button" aria-pressed={goalType === t} onClick={() => setGoalType(t as any)} className={`flex-1 py-5 rounded-[25px] font-black uppercase text-[10px] tracking-widest transition-all focus-visible:ring-2 ring-emerald-500 outline-none ${goalType === t ? 'bg-slate-900 text-white shadow-lg' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}>
                   {t === 'count' ? '# Workouts' : t}
                 </button>
               ))}
            </div>
          ...
          <div className="space-y-3 z-30">
            <label htmlFor={goalType === 'duration' ? hoursId : targetId} className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest">{goalType === 'distance' ? `Target Distance (${distUnit})` : goalType === 'duration' ? 'Target Duration' : 'Number of Workouts'}</label>
            {goalType === 'duration' ? (
              <div className="flex gap-2">
                 <input id={hoursId} aria-label="Hours" className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus:ring-2 ring-emerald-500 outline-none" placeholder="HH" value={durationDetails.h} onChange={e => setDurationDetails({...durationDetails, h: e.target.value})} />
                 <span className="self-center font-black text-slate-300" aria-hidden="true">:</span>
                 <input id={minutesId} aria-label="Minutes" className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus:ring-2 ring-emerald-500 outline-none" placeholder="MM" value={durationDetails.m} onChange={e => setDurationDetails({...durationDetails, m: e.target.value})} />
                 <span className="self-center font-black text-slate-300" aria-hidden="true">:</span>
                 <input id={secondsId} aria-label="Seconds" className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus:ring-2 ring-emerald-500 outline-none" placeholder="SS" value={durationDetails.s} onChange={e => setDurationDetails({...durationDetails, s: e.target.value})} />
              </div>
            ) : (
              <input id={targetId} type="number" className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-lg focus:ring-2 ring-emerald-500 outline-none" placeholder={goalType === 'distance' ? "e.g. 50" : "e.g. 10"} value={target} onChange={e => setTarget(e.target.value)} />
            )}
          </div>
          <div className="space-y-3 z-30">
            <label htmlFor={startDateId} className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest">Start Date</label>
            <input id={startDateId} type="date" className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-slate-900 focus:ring-2 ring-emerald-500 outline-none" value={startDate} onChange={e => setStartDate(e.target.value)} />
          </div>
        </div>

        <div className="mt-10 pt-8 border-t border-slate-100">
           <p className="text-center text-slate-500 font-bold mb-8 text-sm">{getSummary()}</p>
           <button onClick={handleSubmit} disabled={isSubmitting} className="w-full py-6 bg-emerald-400 text-slate-900 rounded-[25px] font-black uppercase text-[11px] tracking-[0.4em] shadow-xl hover:scale-[1.02] transition-transform flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed focus-visible:ring-2 ring-emerald-500 outline-none">
  ...
  return (
    <div className="space-y-12 md:space-y-16 animate-fade-in pb-32 px-4 md:px-0">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end border-b border-slate-100 pb-8 gap-8">
        ...
        <button onClick={() => setShowCreate(true)} className="w-full md:w-auto bg-emerald-400 text-slate-900 px-10 py-5 rounded-[30px] font-black uppercase text-[10px] tracking-[0.3em] hover:scale-105 transition-all shadow-xl flex items-center justify-center gap-3 focus-visible:ring-2 ring-emerald-500 outline-none">
          <Plus size={18} aria-hidden="true" /> Create A Goal
        </button>
      </div>
      ...
      <div className="space-y-8">
        <div className="flex justify-between items-center">
           <div className="flex gap-4">
              <button onClick={() => setFilter('Active')} className={`text-[10px] font-black uppercase tracking-widest transition-colors focus-visible:ring-2 ring-emerald-500 outline-none rounded-md px-2 ${filter === 'Active' ? 'text-emerald-500' : 'text-slate-300'}`}>Active</button>
              <button onClick={() => setFilter('Completed')} className={`text-[10px] font-black uppercase tracking-widest transition-colors focus-visible:ring-2 ring-emerald-500 outline-none rounded-md px-2 ${filter === 'Completed' ? 'text-emerald-500' : 'text-slate-300'}`}>Completed</button>
           </div>
        </div>
        ...
        {visibleGoals.length > 0 ? (
             ...
             {visibleGoals.map((g: any) => {
               ...
               <div key={g.id} className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-lg flex flex-col justify-between h-56 relative overflow-hidden group">
                  <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:opacity-20 transition-opacity">
                     <Target size={80} className="text-emerald-500" aria-hidden="true" />
                  </div>
                  <div>
                    <div className="flex justify-between items-start mb-2">
                       ...
                       {filter === 'Active' && stats.isComplete && (
                          <button onClick={() => markCompleted(g)} className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest hover:bg-emerald-200 transition-colors flex items-center gap-1 focus-visible:ring-2 ring-emerald-500 outline-none">
                             <CheckCircle2 size={12} aria-hidden="true"/> Mark Done
                          </button>
                       )}
                    </div>
                  ...
                  <div className="space-y-2 mt-4 relative z-10">
                    <div className="h-2 bg-slate-100 rounded-full overflow-hidden" role="progressbar" aria-valuenow={Math.round(stats.percent)} aria-valuemin={0} aria-valuemax={100}>
                    ...
                    <div className="flex justify-between items-end">
                        ...
                        <button onClick={(e) => {e.stopPropagation(); deleteGoal(g.id);}} aria-label="Delete goal" className="text-slate-300 hover:text-red-500 p-2 rounded-full hover:bg-red-50 transition-all z-20 focus-visible:ring-2 ring-emerald-500 outline-none"><Trash2 size={16} aria-hidden="true"/></button>
                    </div>
                  </div>
               </div>
             )})}
          ...
          <div className="w-full py-20 border-2 border-dashed border-slate-200 rounded-[40px] flex flex-col items-center justify-center text-center space-y-4">
            <div className="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center text-slate-300"><Flag size={24} aria-hidden="true"/></div>
            <div>
            ...
            {filter === 'Active' && (
                <button onClick={() => setShowCreate(true)} className="mt-4 text-[10px] font-black uppercase tracking-widest text-emerald-500 hover:text-emerald-600 underline focus-visible:ring-2 ring-emerald-500 outline-none rounded-md px-1">Create First Goal</button>
            )}
          </div>
        )}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <div key={i} className="bg-white p-8 md:p-10 rounded-[40px] md:rounded-[50px] border border-slate-100 shadow-sm flex flex-col gap-6 hover:shadow-xl transition-all hover:-translate-y-1">
               ...
               <button onClick={() => addPreset(preset.count)} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] tracking-[0.3em] hover:bg-emerald-400 transition-colors focus-visible:ring-2 ring-emerald-500 outline-none">Set Goal</button>
            </div>
          ))}
        </div>
      </div>
