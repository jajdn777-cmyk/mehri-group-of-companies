      ...
      <div className="bg-white p-6 md:p-10 rounded-[30px] md:rounded-[50px] shadow-2xl w-full max-w-4xl relative overflow-visible animate-scale-in my-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              ...
              {isDropdownOpen && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-100 rounded-[25px] shadow-2xl overflow-hidden h-64 flex flex-col z-[100]" role="listbox">
                  ...
                  <div className="overflow-y-auto custom-scrollbar p-2 flex-1">
                     <button role="option" aria-selected={activity === 'Any Activity'} className="w-full text-left px-4 py-2 rounded-lg text-xs font-bold hover:bg-slate-50 text-emerald-600 mb-2" onClick={() => { setActivity('Any Activity'); setIsDropdownOpen(false); }}>★ Any Activity</button>
                    {Object.keys(filteredCategories).map(category => (
                      <div key={category} className="mb-3">
                        ...
                        {filteredCategories[category].map(act => (
                          <button key={act} role="option" aria-selected={activity === act} className="w-full text-left px-4 py-2 rounded-lg text-xs font-bold hover:bg-slate-50 text-slate-600" onClick={() => { setActivity(act); setIsDropdownOpen(false); }}>{act}</button>
                        ))}
                      </div>
                    ))}
                  </div>
          ...
          <div className="space-y-3 z-40">
            <label htmlFor={`${goalTypeId}-count`} className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest cursor-pointer">Goal Type</label>
            <div className="flex gap-2">
               {['count', 'distance', 'duration'].map(t => (
                 <button key={t} id={`${goalTypeId}-${t}`} onClick={() => setGoalType(t as any)} className={`flex-1 py-5 rounded-[25px] font-black uppercase text-[10px] tracking-widest transition-all focus-visible:ring-2 ring-emerald-500 outline-none ${goalType === t ? 'bg-slate-900 text-white shadow-lg' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}>
                   {t === 'count' ? '# Workouts' : t}
                 </button>
               ))}
            </div>
          ...
          <div className="space-y-3 z-30">
            <label htmlFor={goalType === 'duration' ? hoursId : targetId} className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest cursor-pointer">{goalType === 'distance' ? `Target Distance (${distUnit})` : goalType === 'duration' ? 'Target Duration' : 'Number of Workouts'}</label>
            {goalType === 'duration' ? (
              <div className="flex gap-2">
                 <input id={hoursId} aria-label="Hours" className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus-visible:ring-2 ring-emerald-500 outline-none" placeholder="HH" value={durationDetails.h} onChange={e => setDurationDetails({...durationDetails, h: e.target.value})} />
                 <span className="self-center font-black text-slate-300" aria-hidden="true">:</span>
                 <input id={minutesId} aria-label="Minutes" className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus-visible:ring-2 ring-emerald-500 outline-none" placeholder="MM" value={durationDetails.m} onChange={e => setDurationDetails({...durationDetails, m: e.target.value})} />
                 <span className="self-center font-black text-slate-300" aria-hidden="true">:</span>
                 <input id={secondsId} aria-label="Seconds" className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus-visible:ring-2 ring-emerald-500 outline-none" placeholder="SS" value={durationDetails.s} onChange={e => setDurationDetails({...durationDetails, s: e.target.value})} />
              </div>
            ) : (
      ...
      <div className="space-y-8">
        <div className="flex justify-between items-center">
           <div className="flex gap-4">
              <button onClick={() => setFilter('Active')} className={`text-[10px] font-black uppercase tracking-widest transition-colors focus-visible:ring-2 ring-emerald-500 outline-none rounded-sm ${filter === 'Active' ? 'text-emerald-500' : 'text-slate-300'}`}>Active</button>
              <button onClick={() => setFilter('Completed')} className={`text-[10px] font-black uppercase tracking-widest transition-colors focus-visible:ring-2 ring-emerald-500 outline-none rounded-sm ${filter === 'Completed' ? 'text-emerald-500' : 'text-slate-300'}`}>Completed</button>
           </div>
        </div>
        ...
        {visibleGoals.length > 0 ? (
               ...
               <div key={g.id} className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-lg flex flex-col justify-between h-56 relative overflow-hidden group">
                  ...
                  <div className="space-y-2 mt-4 relative z-10">
                    ...
                    <div className="flex justify-between items-end">
                        ...
                        <button onClick={(e) => {e.stopPropagation(); deleteGoal(g.id);}} aria-label="Delete goal" className="text-slate-300 hover:text-red-500 p-2 rounded-full hover:bg-red-50 transition-all z-20 focus-visible:ring-2 ring-red-500 outline-none"><Trash2 size={16} aria-hidden="true" /></button>
                    </div>
                  </div>
               </div>
             )})}
          ...
          <div className="w-full py-20 border-2 border-dashed border-slate-200 rounded-[40px] flex flex-col items-center justify-center text-center space-y-4">
            ...
            {filter === 'Active' && (
                <button onClick={() => setShowCreate(true)} className="mt-4 text-[10px] font-black uppercase tracking-widest text-emerald-500 hover:text-emerald-600 underline focus-visible:ring-2 ring-emerald-500 outline-none rounded-sm">Create First Goal</button>
            )}
          </div>
        )}
      </div>
