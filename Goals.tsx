...
const CreateGoalModal = ({ onClose, onSubmit, userPreferences }: any) => {
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-[6000] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md overflow-y-auto"
    >
      <div className="bg-white p-6 md:p-10 rounded-[30px] md:rounded-[50px] shadow-2xl w-full max-w-4xl relative overflow-visible animate-scale-in my-8">
        <button
          ...
          className="absolute top-6 right-6 md:top-8 md:right-8 text-slate-300 hover:text-emerald-500 transition-colors focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none rounded-lg"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              ...
              {isDropdownOpen && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-100 rounded-[25px] shadow-2xl overflow-hidden h-64 flex flex-col z-[100]">
                  ...
                  <div className="overflow-y-auto custom-scrollbar p-2 flex-1">
                     <button
                      className="w-full text-left px-4 py-2 rounded-lg text-xs font-bold hover:bg-slate-50 text-emerald-600 mb-2 focus-visible:ring-2 ring-emerald-500 outline-none"
                    ...
                    {Object.keys(filteredCategories).map(category => (
                      <div key={category} className="mb-3">
                        ...
                        {filteredCategories[category].map(act => (
                          <button
                            key={act}
                            className="w-full text-left px-4 py-2 rounded-lg text-xs font-bold hover:bg-slate-50 text-slate-600 focus-visible:ring-2 ring-emerald-500 outline-none"
          ...
          <div className="space-y-3 z-30">
            ...
            {goalType === 'duration' ? (
              <div className="flex gap-2">
                 <input
                  aria-label="Hours"
                  className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus:ring-2 ring-emerald-500 outline-none"
                  placeholder="HH"
                  value={durationDetails.h}
                  onChange={e => setDurationDetails({...durationDetails, h: e.target.value})}
                />
                 ...
                 <input
                  ...
                  className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus:ring-2 ring-emerald-500 outline-none"
                  placeholder="MM"
                  value={durationDetails.m}
                  onChange={e => setDurationDetails({...durationDetails, m: e.target.value})}
                />
                 <span className="self-center font-black text-slate-300">:</span>
                 <input
                  aria-label="Seconds"
                  className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-center focus:ring-2 ring-emerald-500 outline-none"
                  placeholder="SS"
                  value={durationDetails.s}
                  onChange={e => setDurationDetails({...durationDetails, s: e.target.value})}
                />
              </div>
            ) : (
              <input
                type="number"
                className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-lg focus:ring-2 ring-emerald-500 outline-none"
          ...
          <div className="space-y-3 z-30">
            ...
            <input
              type="date"
              id={startDateId}
              className="w-full bg-slate-50 p-5 rounded-[25px] font-bold text-slate-900 focus:ring-2 ring-emerald-500 outline-none"
              value={startDate}
    ...
    <div className="space-y-12 md:space-y-16 animate-fade-in pb-32 px-4 md:px-0">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end border-b border-slate-100 pb-8 gap-8">
        ...
        <button
          ...
          className="w-full md:w-auto bg-emerald-400 text-slate-900 px-10 py-5 rounded-[30px] font-black uppercase text-[10px] tracking-[0.3em] hover:scale-105 transition-all shadow-xl flex items-center justify-center gap-3 focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none"
        >
        ...
        <div className="flex justify-between items-center">
           <div className="flex gap-4">
              <button
                ...
                className={`text-[10px] font-black uppercase tracking-widest transition-colors focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none rounded-lg px-2 py-1 ${filter === 'Active' ? 'text-emerald-500' : 'text-slate-300'}`}
              >
                Active
              </button>
              <button
                onClick={() => setFilter('Completed')}
                className={`text-[10px] font-black uppercase tracking-widest transition-colors focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none rounded-lg px-2 py-1 ${filter === 'Completed' ? 'text-emerald-500' : 'text-slate-300'}`}
              >
                Completed
              </button>
           </div>
        ...
        {visibleGoals.length > 0 ? (
                    ...
                    <div className="flex justify-between items-start mb-2">
                       ...
                       {filter === 'Active' && stats.isComplete && (
                          <button
                            ...
                            className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest hover:bg-emerald-200 transition-colors flex items-center gap-1 focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none"
                          >
                  ...
                  <div className="space-y-2 mt-4 relative z-10">
                    ...
                    <div className="flex justify-between items-end">
                        ...
                        <button
                          ...
                          className="text-slate-300 hover:text-red-500 p-2 rounded-full hover:bg-red-50 transition-all z-20 focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none"
                        >
          ...
          <div className="w-full py-20 border-2 border-dashed border-slate-200 rounded-[40px] flex flex-col items-center justify-center text-center space-y-4">
            ...
            {filter === 'Active' && (
                <button
                  onClick={() => setShowCreate(true)}
                  className="mt-4 text-[10px] font-black uppercase tracking-widest text-emerald-500 hover:text-emerald-600 underline focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none rounded-lg px-2 py-1"
                >
                  Create First Goal
                </button>
            )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <div key={i} className="bg-white p-8 md:p-10 rounded-[40px] md:rounded-[50px] border border-slate-100 shadow-sm flex flex-col gap-6 hover:shadow-xl transition-all hover:-translate-y-1">
               ...
               <button
                onClick={() => addPreset(preset.count)}
                className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] tracking-[0.3em] hover:bg-emerald-400 transition-colors focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none"
              >
                Set Goal
              </button>
            </div>
