...
import { MealAIView } from './MealAI.tsx';
...
const App = () => {
  const getInitialState = () => {
    ...
    const route = VALID_ROUTES[path] || { view: 'landing', dashView: 'dashboard' };
  ...
  useEffect(() => {
    const { data: authListener } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if ((event === 'SIGNED_IN' || event === 'INITIAL_SESSION') && session?.user) {
        ...
        try {
            ...
            if (!profile) {
               ...
               const { data: newProfile } = await supabase.from('profiles').insert({
                   id: user.id, email: user.email, full_name: metadata.full_name, username: newUsername, units: 'metric', join_date: new Date().toISOString()
               }).select().single();
               if (newProfile) { profile = newProfile; setIsNewUserFlow(true); }
            }
            ...
            if (profile) {
               ...
               if (!profile.weight || !profile.height) { handleTransition('specs', undefined, undefined, true); }
               else {
                  if (!hasLoadedSession.current) { await loadUserData(profile.username); }
        ...
        finally { hasLoadedSession.current = true; setIsCheckingAuth(false); }
      } else if (event === 'SIGNED_OUT' || (event === 'INITIAL_SESSION' && !session)) {
         const hasData = userHandle || localStorage.getItem('mehri_handle');
         const shouldRedirect = !isPublic && !hasData;
         if (event === 'SIGNED_OUT' || shouldRedirect) {
            if (!isPublic || (currentPath === "/" && viewRef.current === "main")) {
                handleTransition("landing", "dashboard", "/", true);
            }
            if (event === 'SIGNED_OUT') {
                localStorage.clear(); setWorkouts([]); setUserName(''); setUserHandle(''); hasLoadedSession.current = false;
            }
         }
    ...
    try {
        ...
        if (response?.status === 'success' && response?.data) {
           ...
           if (d.user) {
              ...
              const newProfile = {
                 ...userProfile, username: d.user.username, email: d.user.email,
                 firstName: d.user.auth.name ? d.user.auth.name.split(' ')[0] : '',
                 lastName: d.user.auth.name ? d.user.auth.name.split(' ').slice(1).join(' ') : '',
                 joinDate: d.user.auth.joinDate || '1/27/2026', ...d.user.profile,
                 currentStreak: d.user.streaks?.current || 0
              };
              setUserProfile(newProfile);
              localStorage.setItem('mehri_profile', JSON.stringify(newProfile));
              if (d.user.specs) { setUserSpecs(d.user.specs); localStorage.setItem('mehri_specs', JSON.stringify(d.user.specs)); }
              if (d.user.preferences) { setUserPreferences(d.user.preferences); localStorage.setItem('mehri_preferences', JSON.stringify(d.user.preferences)); }
           }
           const fixDate = (item: any) => item.date || getLocalTodayStr();
  ...
  const handleTransition = (nextView: any, nextDashView?: any, customText?: string, skipLoader?: boolean) => {
    if (!skipLoader && !isCheckingAuth) { setLoadingText(customText || "Loading..."); setIsLoading(true); }
    const executeTransition = () => {
        if (nextView === 'shop') { setShowShop(true); }
        else {
            let path = '/';
            ...
            if (nextView === 'main') { path = '/' + (nextDashView || 'dashboard'); }
            try { window.history.pushState({}, '', path); } catch (e) {}
            setView(nextView); if (nextDashView) setDashView(nextDashView);
            window.scrollTo({ top: 0, behavior: 'smooth' });
        }
        if (!skipLoader) setIsLoading(false);
    };
    if (skipLoader || isCheckingAuth) { executeTransition(); } else { setTimeout(executeTransition, 500); }
  };
  ...
  const handleAuthComplete = async (data: any) => {
      ...
      try {
          ...
          if (profile && (!profile.weight || !profile.height)) { handleTransition('specs', undefined, undefined, true); }
          else { handleTransition('main', 'dashboard', undefined, true); }
      } finally { setIsCheckingAuth(false); hasLoadedSession.current = true; }
  };
  ...
  const handleSignOut = async (remoteLogout = true) => {
    try { if (remoteLogout) { setIsLoading(true); setLoadingText("Securely Logging Out..."); await api("LOGOUT", {}); } } catch (e) {}
    finally { localStorage.clear(); sessionStorage.clear(); window.location.replace("/"); }
  };
  ...
  const handlePublishBlog = async (data: any) => {
    ...
    setBlogs([newPost, ...blogs]); setDashView('blogs');
    try { await api("PUBLISH_BLOG", { ...newPost, username: userHandle }); } catch(e) {}
  };
  ...
  return (
    <div className="min-h-screen bg-[#FCFCFC] font-sans text-slate-900 selection:bg-emerald-100 relative overflow-x-hidden">
      ...
      {!isCheckingAuth && view !== 'privacy' && view !== 'terms' && dashView !== 'write' && (
        <Header currentView={view === 'main' ? dashView : view} onNavigate={navigateTo} onSignOut={() => handleSignOut(true)} onShop={() => setShowShop(true)} userProfile={userProfile} userName={userName} streak={calculateStreak(workouts, userPreferences.restDay)} />
      )}
      <main className={view === 'landing' || view === 'privacy' || view === 'terms' || dashView === 'write' ? '' : 'pt-28 md:pt-52 px-0 md:px-12 pb-20'}>
        {!isCheckingAuth && (
            <>
                ...
                {view === 'specs' && <SpecsSection specs={userSpecs} userPreferences={userPreferences} onComplete={(s: any) => { setUserSpecs(s); api("UPDATE_PROFILE", { username: userHandle, specs: s }); handleTransition('goal', undefined, "Calibrating Bio-Metrics..."); }} />}
                {view === 'goal' && <OnboardingGoalSection onComplete={(goal: string) => { handleTransition('main', 'dashboard', "Finalizing Setup..."); if (isNewUserFlow) setTimeout(() => setShowOnboarding(true), 1500); }} />}
                {view === 'settings' && <div className="max-w-7xl mx-auto px-4 md:px-0"><SettingsView userProfile={userProfile} setUserProfile={setUserProfile} userPreferences={userPreferences} setUserPreferences={setUserPreferences} userSpecs={userSpecs} setUserSpecs={setUserSpecs} workouts={workouts} blogs={blogs} onShop={() => setShowShop(true)} userHandle={userHandle} /></div>}
                {view === 'main' && (
                  <div className={dashView === 'write' ? '' : 'max-w-7xl mx-auto'}>
                    {dashView === 'dashboard' && <DashboardView workouts={workouts} setWorkouts={setWorkouts} userGoals={userGoals} setUserGoals={setUserGoals} routes={routes} userSpecs={userSpecs} userProfile={userProfile} userPreferences={userPreferences} userHandle={userHandle} onForceSync={() => loadUserData(userHandle)} userMeals={userMeals} onNavigate={navigateTo} />}
                    {dashView === 'stats' && <div className="px-4 md:px-0"><StatsView workouts={workouts} userPreferences={userPreferences} userMeals={userMeals} userSpecs={userSpecs} userProfile={userProfile} /></div>}
