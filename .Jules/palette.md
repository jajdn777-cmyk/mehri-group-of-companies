...
## 2026-06-05 - [Accessible Goal Management & Modal Semantics]
**Learning:** Complex form modals require explicit dialog roles and stable ID linkage via `useId` to ensure screen readers correctly associate labels with their inputs, especially when labels are styled as small uppercase text. Additionally, converting interactive container `div`s to semantic `button`s with `aria-haspopup` and `aria-expanded` significantly improves the accessibility of custom selection components.
**Action:** Consistently use `useId` for form elements and convert all interactive containers to semantic buttons with appropriate ARIA state attributes.
