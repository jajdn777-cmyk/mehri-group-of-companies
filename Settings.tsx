        ...
        <div className="w-full lg:w-[280px] shrink-0 space-y-6 md:space-y-8">
          ...
          <div className="flex flex-row lg:flex-col gap-2 overflow-x-auto lg:overflow-visible pb-4 lg:pb-0">
            {navItems.map((item) => (
              <button
                ...
                className={`flex items-center gap-4 px-6 py-4 rounded-2xl transition-all duration-300 group whitespace-nowrap shrink-0 lg:shrink focus-visible:ring-2 ring-emerald-500 outline-none ${activeTab === item.id ? 'bg-slate-900 text-white shadow-lg' : 'bg-transparent text-slate-500 hover:bg-slate-50'}`}
              >
          ...
          {userProfile.email === ADMIN_EMAIL && (
             <button 
               ...
               className="w-full mt-4 md:mt-8 bg-slate-900 text-[#A7F3D0] border border-[#A7F3D0]/20 px-6 py-4 rounded-2xl flex items-center justify-center gap-3 hover:bg-slate-800 transition-all shadow-[0_0_15px_rgba(167,243,208,0.1)] group"
             >
               ...
               <div className="space-y-6 md:space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                     <div className="space-y-2">
                        ...
                        <div className="relative">
                           <select id={genderId} value={form.gender} onChange={e => setForm({...form, gender: e.target.value})} className="w-full bg-slate-50 border-none text-slate-900 text-sm font-bold rounded-2xl px-5 py-4 focus:ring-2 focus:ring-[#A7F3D0] outline-none appearance-none font-sans">
                  ...
                  <div className="p-4 md:p-6 bg-slate-50 rounded-3xl border border-slate-100 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                     <div className="flex items-center gap-4">
                        ...
                        <div>
                           <label htmlFor={restDayId} className="text-sm font-bold text-slate-900 uppercase tracking-wide cursor-pointer">Weekly Rest Day</label>
                           <p className="text-xs text-slate-400 mt-1">Protects your streak on this day</p>
                        </div>
                     </div>
                     <div className="relative w-full md:w-auto md:min-w-[200px]">
                        <select id={restDayId} value={userPreferences.restDay || ""} onChange={e => setUserPreferences({...userPreferences, restDay: e.target.value})} className="w-full bg-white border-none text-slate-900 text-xs font-bold rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#A7F3D0] outline-none appearance-none font-sans">
               ...
               <div className="space-y-6">
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between p-6 bg-slate-50 rounded-3xl border border-slate-100 gap-4">
                     ...
                     <div className="flex bg-white rounded-lg p-1 border border-slate-200">
                        <button type="button" aria-pressed={units === 'imperial'} onClick={() => setUserPreferences({...userPreferences, units: 'imperial'})} className={`px-4 py-2 rounded-md text-[10px] font-black uppercase tracking-widest transition-all focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none ${units === 'imperial' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>Imperial</button>
                        <button type="button" aria-pressed={units === 'metric'} onClick={() => setUserPreferences({...userPreferences, units: 'metric'})} className={`px-4 py-2 rounded-md text-[10px] font-black uppercase tracking-widest transition-all focus-visible:ring-2 ring-emerald-500 ring-offset-2 outline-none ${units === 'metric' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>Metric</button>
                     </div>
                  </div>
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between p-6 bg-slate-50 rounded-3xl border border-slate-100 gap-4">
                     <div className="flex items-center gap-4"><div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-slate-400 shadow-sm"><Clock size={18}/></div><div><label htmlFor={timezoneId} className="text-sm font-bold text-slate-900 uppercase tracking-wide cursor-pointer">Time Zone</label><p className="text-xs text-slate-400 mt-1">Local time display</p></div></div>
                     <div className="relative w-full md:w-auto md:min-w-[200px]">
                        <select id={timezoneId} value={userPreferences.timezone} onChange={e => setUserPreferences({...userPreferences,timezone: e.target.value})} className="w-full bg-white border-none text-slate-900 text-xs font-bold rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#A7F3D0] outline-none appearance-none font-sans">
