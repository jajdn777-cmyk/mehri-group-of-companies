          ...
          {activeTab === 'profile' && (
            <div className="space-y-8 md:space-y-12 animate-fade-in">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                     <div className="space-y-2">
                        ...
                        <div className="relative">
                           <select id={genderId} value={form.gender} onChange={e => setForm({...form, gender: e.target.value})} className="w-full bg-slate-50 border-none text-slate-900 text-sm font-bold rounded-2xl px-5 py-4 focus-visible:ring-2 focus-visible:ring-emerald-500 outline-none appearance-none font-sans">
               ...
               <div className="pt-4 md:pt-8">
                  <button type="button" onClick={handleSave} className="w-full md:w-auto bg-slate-900 text-white px-10 py-4 rounded-full font-black uppercase text-[10px] tracking-[0.3em] hover:bg-emerald-500 transition-colors shadow-lg flex items-center justify-center gap-3"><Save size={16}/> Save Changes</button>
               </div>
            </div>
          )}
          ...
          {activeTab === 'subscription' && (
            <div className="space-y-12 animate-fade-in flex flex-col items-center text-center py-10">
               ...
               <div className="bg-slate-50 p-8 rounded-[30px] border border-slate-100 w-full max-w-xl">
                  ...
                  <button type="button" onClick={onShop} className="w-full py-5 bg-[#A7F3D0] text-slate-900 rounded-2xl font-black uppercase text-xs tracking-[0.3em] hover:brightness-105 transition-all shadow-lg flex items-center justify-center gap-3">Subscribe & Get Tracker <ArrowRight size={16}/></button>
               </div>
            </div>
          )}
               ...
               <div className="space-y-6">
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between p-6 bg-slate-50 rounded-3xl border border-slate-100 gap-4">
                     ...
                     <div className="flex bg-white rounded-lg p-1 border border-slate-200">
                        <button type="button" onClick={() => setUserPreferences({...userPreferences, units: 'imperial'})} className={`px-4 py-2 rounded-md text-[10px] font-black uppercase tracking-widest transition-all ${units === 'imperial' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>Imperial</button>
                        <button type="button" onClick={() => setUserPreferences({...userPreferences, units: 'metric'})} className={`px-4 py-2 rounded-md text-[10px] font-black uppercase tracking-widest transition-all ${units === 'metric' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>Metric</button>
                     </div>
                  </div>
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between p-6 bg-slate-50 rounded-3xl border border-slate-100 gap-4">
  ...
  return (
    <div className="space-y-2 w-full">
      ...
      <input
        id={id}
        type={type}
        className={`w-full bg-slate-50 border-none text-slate-900 text-sm font-bold rounded-2xl px-5 py-4 focus-visible:ring-2 focus-visible:ring-emerald-500 outline-none transition-all font-sans ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
