      ...
      <header className={`fixed top-0 left-0 right-0 h-28 md:h-48 z-[5000] transition-all duration-700 px-4 md:px-8 flex items-start pt-3 md:pt-6 justify-between ${scrolled || isDashboard || currentView === 'auth' || mobileMenuOpen ? 'bg-white/70 backdrop-blur-[12px] border-b border-slate-100/50 shadow-sm' : 'bg-transparent'}`}>
        ...
        <div className="flex items-start gap-4">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="relative flex items-center cursor-pointer transition-all z-50 focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:outline-none rounded-lg"
              onClick={() => {
              ...
              <div className="flex items-center gap-8 ml-6">
                  ...
                  >
                    <button
                      ...
                      className={`text-sm font-black uppercase tracking-[0.2em] flex items-center gap-1 transition-colors whitespace-nowrap focus-visible:text-emerald-500 focus-visible:outline-none ${workoutsHover ? 'text-emerald-500' : 'text-slate-900'}`}
                    >
                  ...
                  >
                     <button
                        ...
                        className={`text-sm font-black uppercase tracking-[0.2em] flex items-center gap-1 transition-colors whitespace-nowrap relative focus-visible:text-emerald-500 focus-visible:outline-none ${almaHover ? 'text-emerald-500' : 'text-slate-900'}`}
                     >
                        <Brain size={12}/> Alma
                        {hasAlmaNotification && (
                           <span
                            aria-label="1 new notification"
                            className="absolute -top-1 -right-2 w-4 h-4 bg-red-500 text-white text-[8px] flex items-center justify-center rounded-full animate-pulse shadow-sm border border-white"
                           >
                            1
                           </span>
                        )}
